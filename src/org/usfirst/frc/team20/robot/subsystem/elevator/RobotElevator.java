 ...
 * Robot and Sensors code of this particular setup. This class is provided for
 * convenience to avoid locking the IElevator interface to the Robot class and
 * also to provide the default behaviors nonetheless.
 * 
...
public interface RobotElevator extends IElevator, ISubsystem {
	...
	static boolean isElevatorAtTop() {
		return Utils.epsilonEquals(
			Utils.ticksToMeters(Robot.elevatorSC.getEncPosition()),
			Constants.ELEVATOR_HEIGHT, 0.1f);
	}

	/**
	 * Get a state corresponding to
	 * 
	 * @return
	 */
	...
	static ElevatorLevel getElevatorLevel() {
		...
		double distance = Utils
			.ticksToMeters(Robot.elevatorSC.getEncPosition());
		level.height = distance;
		if (Sensors.elevatorLoweredSwitch.get()) {
			level.atLevel = true;
			level.level = 0;
			level.height = 0;
			Robot.elevatorSC.setPosition(0);
		}
		if (isElevatorAtTop()) {
			level.atLevel = true;
		...
		for (int i = 0; i < Constants.ELEVATOR_MAX_LEVEL - 1; ++i) {
			double levelHeight = Constants.ELEVATOR_HEIGHT
